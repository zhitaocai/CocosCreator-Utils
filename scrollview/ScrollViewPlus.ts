...
export default class ScrollViewPlus extends cc.ScrollView {
	 ...
	 * @param caculatePosition 是否计算在可视区域中Item的相对位置（可能会相对耗性能）
	 */
				...
				if (caculatePosition) {
					if (itemComponent.isShowing) {
						itemComponent.publishOnPositionChange(
							(childNodeBBox.x - (svBBoxRect.x - childNodeBBox.width / 2)) /
								(childNodeBBox.width + svBBoxRect.width),
							(childNodeBBox.y - (svBBoxRect.y - childNodeBBox.height / 2)) /
								(childNodeBBox.height + svBBoxRect.height)
						);
					}
				}
			} else {
	 ...
	 * @param targetNode 待判断节点
	 */
	public static isNodeVisiable(scrollView: cc.ScrollView, targetNode: cc.Node) {
		...
		return targetNode
			...
			.intersects(
				cc.rect(svLeftBottomPoint.x, svLeftBottomPoint.y, scrollView.node.width, scrollView.node.height)
			);
	}
}
