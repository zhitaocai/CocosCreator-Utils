import SafeAreaAdapter from "./SafeAreaAdapter";
 ...
 */
@ccclass
export default class SafeAreaWidget extends cc.Component {
    @property({
    ...
    onLoad() {
        ...
        if (widget.isAlignTop && this.withInsertTop) {
            ...
            widget.top = -SafeAreaAdapter.screenPxToDesignPx(SafeAreaAdapter.safeArea.safeAreaMarginTop);
            this.node.height += Math.abs(widget.top);
        }
        // 如果对齐下边界，并且包含安全区域到屏幕下边界的缝隙
        if (widget.isAlignBottom && this.withInsertBottom) {
            widget.isAbsoluteBottom = true;
            widget.bottom = -SafeAreaAdapter.screenPxToDesignPx(SafeAreaAdapter.safeArea.safeAreaMarginBottom);
            this.node.height += Math.abs(widget.bottom);
        }
        // 如果对齐左边界，并且包含安全区域到屏幕左边界的缝隙
        if (widget.isAlignLeft && this.withInsertLeft) {
            widget.isAbsoluteLeft = true;
            widget.left = -SafeAreaAdapter.screenPxToDesignPx(SafeAreaAdapter.safeArea.safeAreaMarginLeft);
            this.node.width += Math.abs(widget.left);
        }
        // 如果对齐右边界，并且包含安全区域到屏幕右边界的缝隙
        if (widget.isAlignRight && this.withInsertRight) {
            widget.isAbsoluteRight = true;
            widget.right = -SafeAreaAdapter.screenPxToDesignPx(SafeAreaAdapter.safeArea.safeAreaMarginRight);
